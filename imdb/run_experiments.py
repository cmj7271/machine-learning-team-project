...
def run_experiments(X, y):
    ...
    print("⚠️  이 실험은 상당한 시간이 걸립니다 (약 1-2시간)")
    ...
    if response.lower() == 'y':
        ...
        print("\n✅ 학습 곡선 결과가 'learning_curve_results.json'에 저장되었습니다.")
    else:
        print("⏭️  학습 곡선 실험을 건너뜁니다.")
    
    # ===============================================
    # Experiment 2: 개선된 모델 비교
    # ===============================================
    print("\n" + "="*70)
    print("3. 모델 비교 실험 시작")
    print("="*70)
    
    from improved_models_comparison import (
        compare_models,
        print_comparison_table,
        plot_training_histories
    )
    
    response = input("\n모델 비교 실험을 진행하시겠습니까? (y/n): ")
    ...
    if response.lower() == 'y':
        ...
        # 시각화
        plot_training_histories(comparison_results)
        ...
        for model_name, result in comparison_results.items():
            ...
            print(f"{model_name:<20} Test Accuracy: {test_acc:.4f}")
    else:
        print("⏭️  모델 비교 실험을 건너뜁니다.")
...
def main():
    ...
    ║    2. 개선된 3가지 모델 비교 (MLP, LSTM, Attention)         ║
    ║                                                               ║
    ╚═══════════════════════════════════════════════════════════════╝
    """)
